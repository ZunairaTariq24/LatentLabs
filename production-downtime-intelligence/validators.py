...
def validate_ai_explanation(
    ...
    if len(splits) >= 11:  # header match splits create interleaved elements
        current_sec = None
    ...
    if is_safety:
        ...
        decision_text = sections.get("DECISION", "").lower()
    ...
    # Extract any large numbers (>1000) from text and verify they match Option A, Option B, Net Avoided, or line loss
    valid_numbers = {
    ...
    # Also include parts and labor costs
    ...
    # Find numbers >= 1000 in the AI text (ignoring years like 2026)
    found_nums = re.findall(r"\b(\d{1,3}(?:,\d{3})+|\d{4,})\b", text)
    for num_str in found_nums:
        clean_num = int(num_str.replace(",", ""))
        if clean_num in (2026, 2025, 2024, 100, 50, 25):  # allow common years / capacities
            continue
        # If number is > 1000 and not in our deterministic numbers, flag potential fabrication
