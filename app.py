...
with st.sidebar:
    ...
    view_mode = st.radio(
        ...
        options=[
            "Mode 1: Graphical Factory Topology",
            "Mode 2: Quick Diagnostic Dropdown",
...
if view_mode == "Mode 1: Graphical Factory Topology":
...
elif view_mode == "Mode 2: Quick Diagnostic Dropdown":
    ...
    # Mode 3: AI4I Predictive Maintenance (Random Forest ML)
    st.markdown("---")
    st.subheader("🤖 AI4I Predictive Maintenance (Random Forest ML)")

    # Non-Technical Explanation
    st.markdown("""
    <div style="background:#1e293b; border:1px solid #6366f1; border-radius:10px; padding:18px; margin-bottom:18px;">
        <h4 style="color:#a5b4fc; margin-top:0; font-size:1.15rem;">Prevent Machine Failures Before They Happen</h4>
        <p style="color:#e2e8f0; font-size:0.95rem; line-height:1.6; margin-bottom:8px;">
            Our AI analyzes important machine conditions such as speed, torque, temperature, and tool usage to identify early signs that a machine may be at risk of failure.
        </p>
        <p style="color:#e2e8f0; font-size:0.95rem; line-height:1.6; margin-bottom:10px;">
            Instead of waiting for a machine to break down, the system provides an early warning so maintenance teams can act sooner.
        </p>
        <div style="color:#818cf8; font-weight:700; font-size:0.85rem; text-transform:uppercase; letter-spacing:0.5px;">
            Detect early. Act sooner. Keep production running.
        </div>
    </div>
    """, unsafe_allow_html=True)

    # Model Performance / About the AI Card
    with st.expander("📊 About the AI & Model Performance Benchmark", expanded=False):
        st.markdown("**Predictive prototype trained on the AI4I 2020 benchmark dataset.**")
        st.caption("Random Forest classification pipeline with Scikit-learn.")
        bcol1, bcol2, bcol3, bcol4 = st.columns(4)
        bcol1.metric("ROC-AUC", "0.9662")
        bcol2.metric("Precision (0.40 thr)", "63.3%")
        bcol3.metric("Recall", "73.5%")
        bcol4.metric("F1 Score", "68.0%")

    # Quick Presets for Threshold Verification
    st.markdown("##### Quick Test Cases (Threshold Verification)")
    tcol1, tcol2, tcol3 = st.columns(3)
    with tcol1:
        if st.button("🟢 Case 1: Low Risk (< 20% Prob)", use_container_width=True):
            st.session_state.ml_type = "L"
            st.session_state.ml_air_temp = 298.1
            st.session_state.ml_proc_temp = 308.6
            st.session_state.ml_speed = 1551.0
            st.session_state.ml_torque = 32.8
            st.session_state.ml_wear = 10.0
            st.rerun()

    with tcol2:
        if st.button("🟡 Case 2: Medium Risk (20% – 40% Prob)", use_container_width=True):
            st.session_state.ml_type = "L"
            st.session_state.ml_air_temp = 300.5
            st.session_state.ml_proc_temp = 310.0
            st.session_state.ml_speed = 1400.0
            st.session_state.ml_torque = 62.0
            st.session_state.ml_wear = 120.0
            st.rerun()

    with tcol3:
        if st.button("🔴 Case 3: High Risk (≥ 40% Prob)", use_container_width=True):
            st.session_state.ml_type = "L"
            st.session_state.ml_air_temp = 304.5
            st.session_state.ml_proc_temp = 312.0
            st.session_state.ml_speed = 1200.0
            st.session_state.ml_torque = 75.0
            st.session_state.ml_wear = 235.0
            st.rerun()

    # Machine Sensor Telemetry Inputs
    st.markdown("##### Machine Sensor Telemetry Inputs")
    icol1, icol2, icol3 = st.columns(3)
    type_options = ["L", "M", "H"]
    current_type = st.session_state.get("ml_type", "L")
    type_idx = type_options.index(current_type) if current_type in type_options else 0

    with icol1:
        ml_type_val = st.selectbox(
            "Machine Type (Variant):",
            options=type_options,
            index=type_idx,
            help="Quality variant: L (50%), M (30%), H (20%). Preprocessing handled inside pipeline.",
        )
        ml_air_temp_val = st.number_input(
            "Air Temperature [K]:",
            value=float(st.session_state.get("ml_air_temp", 300.5)),
            min_value=200.0,
            max_value=400.0,
            step=0.1,
            format="%.1f",
    ...
    with icol2:
        ml_proc_temp_val = st.number_input(
            "Process Temperature [K]:",
            value=float(st.session_state.get("ml_proc_temp", 310.0)),
            min_value=200.0,
            max_value=400.0,
            step=0.1,
            format="%.1f",
        )
        ml_speed_val = st.number_input(
            "Rotational Speed [rpm]:",
            value=float(st.session_state.get("ml_speed", 1400.0)),
            min_value=0.0,
            max_value=10000.0,
            step=10.0,
            format="%.0f",
    ...
    with icol3:
        ml_torque_val = st.number_input(
            "Torque [Nm]:",
            value=float(st.session_state.get("ml_torque", 62.0)),
            min_value=0.0,
            max_value=200.0,
            step=0.5,
            format="%.1f",
        )
        ml_wear_val = st.number_input(
            "Tool Wear [min]:",
            value=float(st.session_state.get("ml_wear", 120.0)),
            min_value=0.0,
            max_value=1000.0,
            step=1.0,
            format="%.0f",
    ...
    btn_analyze = st.button("🔍 Run Predictive Maintenance Model (Random Forest)", type="primary", use_container_width=True)
    ...
    if btn_analyze:
        ...
        with st.spinner("Executing Random Forest pipeline on machine sensor telemetry..."):
            try:
                failure_prob = predict_failure_probability(
                    machine_type=ml_type_val,
                    air_temperature=ml_air_temp_val,
                    process_temperature=ml_proc_temp_val,
                    rotational_speed=ml_speed_val,
                    torque=ml_torque_val,
                    tool_wear=ml_wear_val,
                )

                decision_dict = evaluate_maintenance_decision(failure_prob)
                ...
                st.session_state.ml_result = {
                    ...
                    "inputs": {
                        "Type": ml_type_val,
                        "Air temperature [K]": ml_air_temp_val,
                        "Process temperature [K]": ml_proc_temp_val,
                        "Rotational speed [rpm]": ml_speed_val,
                        "Torque [Nm]": ml_torque_val,
                        "Tool wear [min]": ml_wear_val,
                    }
                }
                ...
                st.session_state.audit_log.insert(0, {
                    ...
                    "machine_id": f"Type-{ml_type_val}",
                    "error_id": f"ML-PRED ({decision_dict['formatted_probability']})",
                    "decision": f"{decision_dict['risk_display']} : {decision_dict['decision']}",
                    "net_avoided_loss": 0.0,
                    "safety_override": decision_dict["risk_level"] == "HIGH",
                })

            except Exception as e:
                st.error(f"ML Pipeline Execution Error: {str(e)}")

    # Display Prediction Results
    if "ml_result" in st.session_state:
        res = st.session_state.ml_result["decision"]
        inps = st.session_state.ml_result["inputs"]
        ...
        with rcol1:
            ...
            <div class="metric-card">
                ...
                <div class="metric-val" style="color:{res['status_color']};">{res['formatted_probability']}</div>
                <div class="metric-sub">Threshold: Low &lt;20% | Med 20-40% | High &ge;40%</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with rcol2:
            ...
            <div class="metric-card">
                ...
                <div class="metric-val" style="color:{res['status_color']};">{res['risk_display']}</div>
                <div class="metric-sub">Deterministic rule classification</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with rcol3:
            st.markdown(f"""
            <div class="metric-card">
                <div class="metric-title">Maintenance Decision</div>
                <div class="metric-val" style="color:{res['status_color']}; font-size:1.3rem;">{res['decision']}</div>
                <div class="metric-sub">Deterministic operational gate</div>
            </div>
            """, unsafe_allow_html=True)

        # Recommended Action Banner
        st.markdown(f"""
        <div style="background:#1e293b; border-left:4px solid {res['status_color']}; padding:14px; border-radius:6px; margin-top:14px;">
            <div style="font-size:0.8rem; font-weight:700; color:#94a3b8; text-transform:uppercase;">Recommended Action</div>
            <div style="font-size:1.05rem; font-weight:600; color:#f8fafc; margin-top:4px;">{res['recommendation']}</div>
        </div>
        """, unsafe_allow_html=True)

        # Key Model Factors Card
        st.markdown("##### Key model factors")
        st.caption("Note: These are predictive correlation inputs from the trained model, not causal assertions.")
        fcol1, fcol2, fcol3, fcol4, fcol5 = st.columns(5)
        fcol1.info(f"**Torque:**\n{inps['Torque [Nm]']} Nm")
        fcol2.info(f"**Rotational Speed:**\n{inps['Rotational speed [rpm]']} rpm")
        fcol3.info(f"**Tool Wear:**\n{inps['Tool wear [min]']} min")
        fcol4.info(f"**Air Temp:**\n{inps['Air temperature [K]']} K")
        fcol5.info(f"**Process Temp:**\n{inps['Process temperature [K]']} K")

        # Grok AI Explanation Layer
        st.markdown("---")
        st.subheader("🤖 AI Explanation")
        st.caption("Grounded AI explanation synthesized via Groq LLM without modifying model probabilities or decisions.")

        from ai_engine import generate_predictive_maintenance_ai_explanation

        with st.spinner("Synthesizing grounded explanation via Groq..."):
            exp_text, is_ai, notice = generate_predictive_maintenance_ai_explanation(
                machine_data=inps,
                decision_result=res,
    ...
    # Show Shift Handover Audit Log
    st.markdown("---")
    st.subheader("📋 Shift Handover & Decision Audit Log")
    ...
    <div style="text-align:center; color:#64748b; font-size:0.85rem;">
        Production Downtime & Maintenance Decision Intelligence &bull; AI4I Predictive Maintenance Engine<br>
        Scikit-Learn Random Forest Pipeline &bull; Groq AI Explanation Layer &bull; Zero Hallucinations Guaranteed
    </div>
    """, unsafe_allow_html=True)
